{
  ...
  expandDependencies = packages: let
    withDeps = p:
      map (
        x:
          if x == null
          then []
          else [x] ++ withDeps x.propagatedBuildInputs
      ) (flatten p);
  in (unique (filter (d: d ? ename) (flatten (withDeps packages))));

  install = repo: packages: let
    installPkg = repo: pkg: ''
      REPO=${repo}
  ...
  parsePackagesJSON = json: let
    list = importJSON json;
  in
    map (x:
      if epkgs ? "${x}"
      then epkgs.${x}
      else if abortOnNotFound
      then abort "Package not available: ${x}"
    ...
    stdenv.mkDerivation {
      ...
      installPhase = ''
        ...
        emacs -q      \
              ...
              --load=${./setup.el} \
              ${concatMapStringsSep " " (f: "--load=${f}") emacsLoadFiles} \
